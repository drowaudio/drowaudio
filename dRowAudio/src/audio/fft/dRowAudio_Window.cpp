...
void Window::setUpWindowBuffer()
{
	...
	{
		case Rectangular:
			applyRectangularWindow(bufferSample, bufferSize);
			break;
		case Hann:
			applyHannWindow(bufferSample, bufferSize);
			break;
		case Hamming:
			applyHammingWindow(bufferSample, bufferSize);
			break;
		case Cosine:
			applyCosineWindow(bufferSample, bufferSize);
			break;
		case Lanczos:
			applyLanczosWindow(bufferSample, bufferSize);
			break;
		case ZeroEndTriangle:
			applyZeroEndTriangleWindow(bufferSample, bufferSize);
			break;
		case NonZeroEndTriangle:
			applyNonZeroEndTriangleWindow(bufferSample, bufferSize);
			break;
		case Gaussian:
			applyGaussianWindow(bufferSample, bufferSize);
			break;
		case BartlettHann:
			applyBartlettHannWindow(bufferSample, bufferSize);
			break;
		case Blackman:
			applyBlackmanWindow(bufferSample, bufferSize);
			break;
		case Nuttall:
			applyNuttallWindow(bufferSample, bufferSize);
			break;
		case BlackmanHarris:
			applyBlackmanHarrisWindow(bufferSample, bufferSize);
			break;
		case BlackmanNuttall:
			applyBlackmanNuttallWindow(bufferSample, bufferSize);
			break;
		case FlatTop:
			applyFlatTopWindow(bufferSample, bufferSize);
			break;
		default:
			applyRectangularWindow(bufferSample, bufferSize);
			break;
...
{
	printf("Apply Rectangular window to audio buffer:\n");
...
void Window::applyHannWindow(float *samples, const int numSamples)
{
	...
	for(int i = 0; i < numSamples; i++)
	{
		...
		float window = 0.5 * (1.0 - cos((double)i * oneOverSizeMinusOne * twoTimesPi));
		samples[i] *= window;
		windowFactor += window;
	}
	windowFactor *= oneOverSize;
...
{
	printf("Apply Hamming window to audio buffer:\n");
	const double oneOverSize = (1.0 / numSamples);
	const double oneOverSizeMinusOne = 1.0 / (numSamples - 1.0);
	windowFactor = 0.0;
	for(int i = 0; i < numSamples; i++)
	{
		...
		float window = 0.54 - 0.46 * (cos(twoTimesPi * (double)i * oneOverSizeMinusOne));
		samples[i] *= window;
		windowFactor += window;
	}
	windowFactor *= oneOverSize;	
...
{
	printf("Apply Cosine window to audio buffer:\n");
	const double oneOverSize = (1.0 / numSamples);
	const double oneOverSizeMinusOne = 1.0 / (numSamples - 1.0);
	windowFactor = 0.0;
	for(int i = 0; i < numSamples; i++)
	{
		...
		float window = sin(double_Pi * (double)i * oneOverSizeMinusOne);
		samples[i] *= window;
		windowFactor += window;
	}
	windowFactor *= oneOverSize;		
...
{
	printf("Apply Lanczos window to audio buffer:\n");
	const double oneOverSize = (1.0 / numSamples);
	const double oneOverSizeMinusOne = 1.0 / (numSamples - 1.0);
	windowFactor = 0.0;
	for(int i = 0; i < numSamples; i++)
	{
		...
		float window = sincPi((2.0 * (double)i * oneOverSizeMinusOne)-1.0);
		samples[i] *= window;
		windowFactor += window;
	}
	windowFactor *= oneOverSize;			
...
void Window::applyZeroEndTriangleWindow(float *samples,  const int numSamples)
{
	...
	for(int i = 0; i < numSamples; i++)
	{
		...
		float window = (2.0 * oneOverSizeMinusOne) * ((sizeMinusOne*0.5) - fabs(double(i) - (sizeMinusOne * 0.5)));
		samples[i] *= window;
		windowFactor += window;
	}
	windowFactor *= oneOverSize;			
...
void Window::applyNonZeroEndTriangleWindow(float *samples,  const int numSamples)
{
	...
	{
		// Triangle window equation
		float window = (2.0 * oneOverSize) * ((numSamples*0.5) - fabs(double(i) - (sizeMinusOne * 0.5)));
		samples[i] *= window;
		windowFactor += window;
	}
	windowFactor *= oneOverSize;			
...
void Window::applyGaussianWindow(float *samples,  const int numSamples)
{
	...
	for(int i = 0; i < numSamples; i++)
	{
		...
		float window = exp(-0.5 * squareNumber(((double)i - sizeMinusOne * 0.5)/(sigma * sizeMinusOne * 0.5)));
		samples[i] *= window;
		windowFactor += window;
	}
	windowFactor *= oneOverSize;				
...
{
	printf("Apply Bartlett Hann window to audio buffer:\n");
	...
	for(int i = 0; i < numSamples; i++)
	{
		...
		float window = a0 - a1 * fabs((double)i*oneOverSizeMinusOne - 0.5) - a2 * cos(twoTimesPi * (double)i * oneOverSizeMinusOne);
		samples[i] *= window;
		windowFactor += window;
	}
	windowFactor *= oneOverSize;			
...
{
	DBG("Apply Blackman window to audio buffer:\n");
	
	const float alpha = 0.16f;
	const float a0 = (1.0f - alpha) * 0.5;
	const float a1 = 0.5f;
	const float a2 = alpha * 0.5;
	...
	for(int i = 0; i < numSamples; i++)
	{
		...
		float window = a0 - a1 * cos(twoTimesPi * (double)i * oneOverSizeMinusOne) + a2 * cos(fourTimesPi * (double)i * oneOverSizeMinusOne);
		samples[i] *= window;
		windowFactor += window;
	}
	windowFactor *= oneOverSize;			
...
{
	DBG("Apply Nuttall window to audio buffer:\n");
	...
	for(int i = 0; i < numSamples; i++)
	{
		...
		float window = a0 - a1 * cos(twoTimesPi * (double)i * oneOverSizeMinusOne)
		+ a2 * cos(fourTimesPi * (double)i * oneOverSizeMinusOne)
		- a3 * cos(sixTimesPi * (double)i * oneOverSizeMinusOne);
		samples[i] *= window;
		windowFactor += window;
	}
	windowFactor *= oneOverSize;				
...
{
	DBG("Apply Blackman-Harris window to audio buffer:\n");
...
{
	DBG("Apply Blackman-Nuttall window to audio buffer:\n");
...
{
	DBG("Apply Flat Top window to audio buffer:\n");
	...
	for(int i = 0; i < numSamples; i++)
	{
		...
		+ a2 * cos(fourTimesPi * (double)i * oneOverSizeMinusOne)
		- a3 * cos(sixTimesPi * (double)i * oneOverSizeMinusOne);
		+ a4 * cos(2.0 * fourTimesPi * (double)i * oneOverSizeMinusOne);
		samples[i] *= window;
		windowFactor += window;
	}
	windowFactor *= oneOverSize;				
