									...
									float& lowestRight, float& highestRight)
{
    ...
    if (reader.usesFloatingPointData)
    {
        ...
        while (numSamples > 0)
        {
			...
			// filter here
			filter.processSamples(reinterpret_cast<float*> (tempBuffer[0]), numToDo);
    ...
    else
    {
        ...
        while (numSamples > 0)
        {
			...
			// filter here
			filter.processSamples((tempBuffer[0]), numToDo);
...
//==============================================================================
struct FilteredAudioThumbnail::MinMaxValue
{
    char minValue;
    char maxValue;
...
//==============================================================================
class FilteredAudioThumbnail::LevelDataSource   :	public TimeSliceClient,
													public Timer
{
public:
    LevelDataSource (FilteredAudioThumbnail& owner_, AudioFormatReader* newReader, int64 hash)
        : lengthInSamples (0), numSamplesFinished (0), sampleRate (0), numChannels (0),
          hashCode (hash), owner (owner_), reader (newReader)
    {
    }

    LevelDataSource (FilteredAudioThumbnail& owner_, InputSource* source_)
        : lengthInSamples (0), numSamplesFinished (0), sampleRate (0), numChannels (0),
    ...
    void initialise (int64 numSamplesFinished_)
    {
        ...
        if (reader != 0)
        {
			...
			//drow
			owner.filterSetup.setUpFilter(owner.filter, reader->sampleRate);
    ...
    void getLevels (int64 startSample, int numSamples, Array<float>& levels)
    {
        ...
        {
            float l[4] = { 0 };
            readMaxLevelsFiltering (*reader, owner.filter, startSample, numSamples, l[0], l[1], l[2], l[3]);
...
//                                           lowestLeft, highestLeft, lowestRight, highestRight);
                    readMaxLevelsFiltering (*reader, owner.filter,
 ...
 */
class FilteredAudioThumbnail::ThumbData
{
public:
...
//==============================================================================
class FilteredAudioThumbnail::CachedWindow
{
public:
    CachedWindow()
...
//==============================================================================
FilteredAudioThumbnail::FilteredAudioThumbnail (const int originalSamplesPerThumbnailSample,
												AudioFormatManager& formatManagerToUse_,
												MultipleAudioThumbnailCache& cacheToUse,
												BiquadFilterSetup filterSetupToUse)
:	formatManagerToUse (formatManagerToUse_),
    ...
    numChannels (0),
    sampleRate (0),
	filterSetup(filterSetupToUse)
{
//	smoothingFilter.makeLowPass(originalSamplesPerThumbnailSample, originalSamplesPerThumbnailSample * 0.1);
}

FilteredAudioThumbnail::~FilteredAudioThumbnail()
{
    clear();
}

void FilteredAudioThumbnail::clear()
{
    source = 0;
...
void FilteredAudioThumbnail::reset (int newNumChannels, double newSampleRate, int64 totalSamplesInSource)
{
    clear();
...
void FilteredAudioThumbnail::createChannels (const int length)
{
...
//==============================================================================
void FilteredAudioThumbnail::loadFrom (InputStream& input)
{
    clear();
...
void FilteredAudioThumbnail::saveTo (OutputStream& output) const
{
    const ScopedLock sl (lock);
...
//==============================================================================
bool FilteredAudioThumbnail::setDataSource (LevelDataSource* newSource)
{
...
bool FilteredAudioThumbnail::setSource (InputSource* const newSource)
{
    clear();
...
void FilteredAudioThumbnail::setReader (AudioFormatReader* newReader, int64 hash)
{
    clear();
...
int64 FilteredAudioThumbnail::getHashCode() const
{
    return source == 0 ? 0 : source->hashCode;
}

void FilteredAudioThumbnail::addBlock (const int64 startSample, const AudioSampleBuffer& incoming,
...
void FilteredAudioThumbnail::setLevels (const MinMaxValue* const* values, int thumbIndex, int numChans, int numValues)
{
    const ScopedLock sl (lock);
...
//==============================================================================
int FilteredAudioThumbnail::getNumChannels() const throw()
{
    return numChannels;
}

double FilteredAudioThumbnail::getTotalLength() const throw()
{
    return totalSamples / sampleRate;
}

bool FilteredAudioThumbnail::isFullyLoaded() const throw()
{
    return numSamplesFinished >= totalSamples - samplesPerThumbSample;
}

int64 FilteredAudioThumbnail::getNumSamplesFinished() const throw()
{
    return numSamplesFinished;
}

float FilteredAudioThumbnail::getApproximatePeak() const
{
    int peak = 0;
...
void FilteredAudioThumbnail::drawChannel (Graphics& g, const juce::Rectangle<int>& area, double startTime,
...
void FilteredAudioThumbnail::drawChannels (Graphics& g, const juce::Rectangle<int>& area, double startTimeSeconds,
