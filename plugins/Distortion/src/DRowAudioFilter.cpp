...
/*	void init(const String& name_, ParameterUnit unit_, String description_,
	...
	params[PRE].init(parameterNames[PRE], UnitHertz, T("Changes the input filtering"),
					 500.0, 50.0, 5000.0, 500.0);
	params[PRE].setSkewFactor(0.5);
	params[PRE].setStep(1.0);
	params[PRE].setUnitSuffix(T(" Hz"));
	params[POST].init(parameterNames[POST], UnitHertz, T("Changes the output filtering"),
					  500.0, 50.0, 5000.0, 500.0);
	params[POST].setSkewFactor(0.5);
	params[POST].setStep(1.0);
	params[POST].setUnitSuffix(T(" Hz"));
	params[INGAIN].init(parameterNames[INGAIN], UnitGeneric, T("Changes the distortin ammount"),
						1.0, 0.0, 15.0, 1.0);
	params[OUTGAIN].init(parameterNames[OUTGAIN], UnitGeneric, T("Changes the output level"),
						 1.0, 0.0, 2.0, 1.0);
	params[COLOUR].init(parameterNames[COLOUR], UnitGeneric, T("Changes the colour of the distortion"),
						0.1, 0.0, 1.0, 0.1);
}
